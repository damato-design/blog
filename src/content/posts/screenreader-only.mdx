...
I am a big proponent of component-driven development, and don't agree with the [utility class method of styling](./classname-soup). In component-driven development, you offer blackbox abstractions of common implementations. Meanwhile, the utility class method allows for more customization. In a systematic approach, we want our experiences to be consistent and therefore follow the same patterns and conventions. Distributing these blackboxes that align with guidelines by default makes a consistent experience easy. Whereas, trying to learn and follow guidelines from other resources to implement piecemeal comes with misintereptations and other inconsistencies.

However, in this work there has been one concept that I've stuggled to put into this component-driven ecosystem; _screenreader only_ as it has traditionally existed as a class added to an otherwise benign element.
...
My gut reaction is that it would be odd to offer such a component that simply applies the styles required to make it hidden and for it to be offered as an exception to an otherwise utility-free ecosystem. However, I don't like exceptions, I like thoughtful systems.
...
Further, we expect that accessibility is a quality of components, perhaps not a component itself. As I'm having difficulty thinking about other possible memebers of this family that would also be associated with the `<ScreenreaderOnly/>` component.
...
The `<Text/>` component will be widly more popular than the possible `<ScreenreaderOnly/>` component, which will cause more folks to potentially visit its API and learn about the feature. This being an opportunity to educate when its use is appropriate.
...
In Scott's post, he describes some alternatives to screenreader only; specifically on loophole in particular.
...
In this way, we still need to apply styles to the `.visually-hidden` element in order for it to disappear from view. But this also demonstrates how to connect that content to an interactive element for further context.
...
<Aside>
    It's important to know that `aria-label`, `aria-labelledby` and `aria-describedby` are not valid on all elements. [The general gist is that these are meant for interactive elements or landmark elements.](https://www.tpgi.com/short-note-on-aria-label-aria-labelledby-and-aria-describedby/)

    The interface above _could_ warn if the `ref` is attached to an element that is inappropriate.
</Aside>
