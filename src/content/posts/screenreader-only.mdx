...
There's more about this composition and words about screenreader only versus `aria-label` [in this post](https://gomakethings.com/revisting-aria-label-versus-a-visually-hidden-class/#pairing-a-visually-hidden-class-with-aria-hidden) by [Chris Ferdinandi](https://twitter.com/ChrisFerdinandi).

My gut reaction is that it would be odd to offer such a component that simply applies the styles required to make it hidden and for it to be offered as an exception to an otherwise utility-free ecosystem. However, I don't like exceptions, I strive for thoughtful systems.
...
The `<Text/>` component will be widely more popular than the possible `<ScreenreaderOnly/>` component, which will cause more folks to potentially visit its API and learn about the feature. This being an opportunity to educate when its use is appropriate.
